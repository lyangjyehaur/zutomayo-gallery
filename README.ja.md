...
![Version](https://img.shields.io/badge/version-3.7.16-blue)
...
本プロジェクトは、ZUTOMAYO MV の設定画を一覧できるギャラリーに加え、データ保守用の管理画面とモバイル審査フロントエンドを提供します。メインのフロントエンドは React + TypeScript、バックエンドは Express + TypeScript、主データベースは PostgreSQL（Sequelize + Umzug migrations）です。別の `review-app/` サブアプリは Framework7 React を使い、スマートフォンやタブレット向けに staging / submissions / FanArt / group repair ワークフローを担当します。正式環境のライトボックスは現在 Fancybox を使用し、LightGallery は debug/検証ページのみに残しています。
...
- モバイル審査ワークフロー: `review-app/` がダッシュボード要約、staging、submissions、FanArt、repair、settings を担当
...
| [Framework7 React](https://framework7.io/react/) | 9.0 | `review-app` のモバイル審査 UI |
...
│   └── package.json
│
├── review-app/                  # モバイル審査フロントエンド (Framework7 React)
│   ├── src/pages/               # ダッシュボード / staging / submissions / FanArt / repair / settings
│   ├── src/components/          # AppNavbar、MvSheet、ReviewStateBlock など
│   └── README.md
│
...
**コマンドライン**
- `npm run dev` でメイン frontend と backend を同時起動
- `npm run dev:review` で `review-app` と backend を同時起動
- `npm run dev:all` でメイン frontend・`review-app`・backend を同時起動
- `npm run start:frontend` でメイン frontend のみ起動
- `npm run start:review-app` で `review-app` のみ起動
...
# 2. 方法 A: メインサイトの依存をまとめてインストール（推奨: root + frontend + backend + review-app）
npm run install:all
...
| 名前 | 説明 | 既定値 |
|---|---|---|
| `VITE_API_ORIGIN` | 任意の API origin。未設定時は `review-app/src/lib/api.ts` が自動推論 | なし |
| `VITE_API_ROOT` | ローカル開発でよく使う API root。通常は Vite proxy と組み合わせて `/api` | `/api` |
| `VITE_API_URL` | 完全な API base URL。既定の推論を上書きしたい場合に使用 | なし |

  ...
  const fetchUnorganized = async () => {
    try {
      ...
      if (data.success) {
        ...
        toast.error(formatApiError(data, '載入未整理 Cosplay 失敗'));
      }
    } catch (err: any) {
      toast.error(formatApiError(err, '載入未整理 Cosplay 失敗'));
    }
  };
  ...
  const fetchDeleted = async () => {
    try {
      ...
      if (data.success) {
        ...
        toast.error(formatApiError(data, '載入已捨棄 Cosplay 失敗'));
      }
    } catch (err: any) {
      toast.error(formatApiError(err, '載入已捨棄 Cosplay 失敗'));
    }
  };
  ...
  const fetchFanartsByTag = async (tagId: string) => {
    try {
      ...
      if (data.success) {
        ...
        setTagMedia([]);
        toast.error(formatApiError(data, '載入標籤 Cosplay 失敗'));
      }
    } catch (err: any) {
      toast.error(formatApiError(err, '載入標籤 Cosplay 失敗'));
      setTagMedia([]);
    }
  };
    ...
    <div className="h-full flex bg-background text-foreground overflow-hidden font-mono">
      ...
      <div className="flex-1 overflow-y-auto bg-muted">
        {activeTab === 'unorganized' && (
          <div className="p-6">
            <h2 className="text-2xl font-black uppercase tracking-widest mb-6">未整理的 Cosplay</h2>
        ...
        {activeTab === 'legacy' && (
          <div className="p-6">
            <h2 className="text-2xl font-black uppercase tracking-widest mb-6">舊資料 / 異常 Cosplay</h2>
